...
def ProcessTheClient(connection, address):
    ...
    try:
        ...
        logging.info(f"Processing connection from {address}")
    ...
    with ProcessPoolExecutor(20) as executor:
        ...
        while True:
            try:
                ...
                logging.info(f"Connection #{connection_counter} from {client_address}")
                
                p = executor.submit(ProcessTheClient, connection, client_address)
                the_clients.append(p)
                the_clients = [client for client in the_clients if client.running()]
                
                active_count = len([i for i in the_clients if i.running() == True])
                print(f"Active processes: {active_count}/20 | Connection #{connection_counter} | Time: {time.strftime('%H:%M:%S')}")
...
def main():
    try:
        print("="*60)
        print("HTTP Server with ProcessPoolExecutor")
        print("="*60)
        Server()
    except KeyboardInterrupt:
